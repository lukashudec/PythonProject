...
def format_input(inp_list, inp_bound) -> list:
    result = []
    inp_list = [['0:00', inp_bound[0]]]+inp_list+[[inp_bound[1], '0:00']]
    for i in inp_list:
        h, m = i[0].split(':')
        h1, m1 = i[1].split(':')
...
def squish(inp_list) -> list:
    result = []
    i = 0
    while i < len(inp_list)-1:
        j = i
        while inp_list[i][1] >= inp_list[j + 1][0]:
            inp_list[i][1] = inp_list[j + 1][1]
            j += 1
        result.append(inp_list[i])
        i = j+1
    return result
