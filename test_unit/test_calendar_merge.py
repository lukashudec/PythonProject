...
def test_get_free_block_general():
    assert output == get_free_block(calendar1, bound1, calendar2, bound2),\
        "actual different from expected"


def test_get_free_time():
    output = [[900, 960], [1080, 1110]]
    assert get_free_time(sorted(format_input(calendar1, bound1) + format_input(calendar2, bound2))) == output,\
        "actual different from expected"
